...
async function addProductAction(formData: FormData) {
  ...
  if (!name) {
    redirect('/admin/products?error=name-required')
  }
  ...
  if (photoFiles.length === 0) {
    redirect('/admin/products?error=image-required')
  }
  ...
  try {
    ...
    redirect('/admin/products?error=drive-upload-failed')
  }
  ...
  try {
    ...
    redirect('/admin/products?error=sheet-append-failed')
  }
  ...
  const product = {
    ...
    stock_quantity: sharedInput.stock_quantity,
...
async function syncSheetAction() {
  ...
  try {
    ...
    redirect(`/admin/products?sync=ok&imported=${result?.imported ?? 0}`)
  } catch (e) {
    console.error('Sheet sync failed', e)
    const message = e instanceof Error ? e.message : 'sync_failed'
    redirect(`/admin/products?sync=error&reason=${encodeURIComponent(message)}`)
  }
}
...
export default async function AdminPage({
  ...
  const linkedDriveAccount = process.env.POSTGRES_URL
    ? await getLinkedDriveAccount().catch((error) => {
        console.error('[admin/products] Google Drive status unavailable:', error)
        return null
      })
    : null
