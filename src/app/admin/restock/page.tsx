    ...
    return (
      <main className="min-h-screen bg-[#F8F7FB] flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <Link href="/admin" className="inline-block px-6 py-3 bg-[#6046A3] text-white rounded-lg font-semibold">
            Перейти до входу
          </Link>
        </div>
      </main>
            ...
            {groups.map((g) => (
              <div key={g.name} className="bg-white rounded-2xl border border-[#E5E5E5] p-5 sm:p-6">
                <div className="flex items-center justify-between gap-3 mb-3">
                  ...
                  <span className="text-[12px] px-2 py-[2px] rounded-full bg-[#F5F3FF] text-[#6046A3] font-medium whitespace-nowrap">
                ...
                <ul className="space-y-1">
                  {g.contacts.map((c, i) => (
                    <li key={i} className="flex items-center justify-between text-[14px] border-t border-[#EEE] pt-2 first:border-0 first:pt-0">
                      <a href={c.contact.includes('@') ? `mailto:${c.contact}` : `tel:${c.contact}`} className="text-[#6046A3] hover:underline">
                        {c.contact}
                      </a>
