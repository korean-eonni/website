    ...
    <label className="flex items-center gap-2 cursor-pointer group">
      ...
      <div
        className={`w-[16px] h-[16px] border border-[#BBBBBB] rounded-sm flex items-center justify-center transition-colors peer-focus-visible:ring-2 peer-focus-visible:ring-[#6046A3] peer-focus-visible:ring-offset-1 ${
          checked ? 'bg-[#6046A3] border-[#6046A3]' : 'bg-[#E2F9FF] group-hover:border-[#999]'
        }`}
      >
        {checked && (
        ...
        <div className="relative aspect-square rounded-[12px] sm:rounded-[15px] overflow-hidden bg-white z-[1]">
          ...
          {allImages.length > 1 && (
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-[3]">
              {allImages.map((_, i) => (
                <div key={i} className={`w-1.5 h-1.5 rounded-full transition-colors ${i === hoveredIndex ? 'bg-[#6046A3]' : 'bg-white/70 ring-1 ring-black/10'}`} />
              ))}
            </div>
          )}
          ...
          {comingSoon && (
            <div className="absolute top-3 left-3 z-[4]">
              <span className="h-[26px] px-3 rounded-[8px] bg-[#6046A3] text-white text-[12px] font-semibold flex items-center shadow-[0_3px_10px_rgba(96,70,163,0.35)]">
                Скоро
              </span>
            </div>
          )}
          ...
          {comingSoon && notifyOpen && (
            ...
            >
              {notifyStatus === 'done' ? (
                <p className="text-[13px] text-[#6046A3] font-semibold leading-snug">
                ...
                <form onSubmit={handleNotify} className="w-full max-w-[230px]">
                  ...
                  <input
                    ...
                    className="w-full h-9 px-3 border border-[#CCC] rounded-lg text-[13px] mb-2 outline-none focus:border-[#6046A3]"
                  />
                  <button
                    type="submit"
                    disabled={notifyStatus === 'sending'}
                    className="w-full h-9 rounded-lg bg-[#6046A3] text-white text-[13px] font-semibold hover:bg-[#4D3882] transition-colors disabled:opacity-60"
                  >
          ...
          {comingSoon ? (
            <button
              className="absolute bottom-3 right-3 w-[40px] h-[40px] rounded-lg flex items-center justify-center bg-white hover:bg-[#E2F9FF] text-[#6046A3] transition-colors shadow-sm z-[4]"
            ...
            <button
              className={`absolute bottom-3 right-3 w-[40px] h-[40px] rounded-lg flex items-center justify-center transition-colors shadow-sm disabled:cursor-not-allowed z-[4] ${
                adding ? 'bg-[#6046A3] text-white' : 'bg-white hover:bg-[#E2F9FF] text-black'
              }`}
              ...
              {product.tags
                ...
                .map((tag, i) => (
                  <span
                    ...
                    className="text-[10px] sm:text-[11px] leading-tight px-2 py-[3px] rounded-full bg-[#F5F3FF] text-[#6046A3] cursor-pointer hover:bg-[#E7DEFF] transition-colors"
                  >
                    {tag}
                  </span>
                ))}
              ...
              return (
                <button
                  ...
                  className={`px-4 sm:px-5 h-9 rounded-full text-[13px] sm:text-[14px] font-gilroy font-semibold tracking-wide uppercase transition-colors duration-200 whitespace-nowrap ${
                    active
                      ? 'bg-[#6046A3] text-white shadow-[0_4px_14px_rgba(96,70,163,0.32)]'
