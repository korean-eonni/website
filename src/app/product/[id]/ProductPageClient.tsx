...
function Breadcrumbs({ 
  ...
  const seoCategory = getSeoCategoryForRaw(category)
  ...
  return (
    <nav className="flex items-center gap-2 text-[16px] sm:text-[18px] leading-[24px] mb-6 flex-wrap">
      ...
      {category && (
        <>
          <Link 
            href={seoCategory ? `/category/${seoCategory.slug}` : '/catalog'}
            className="font-semibold text-black hover:text-[#7C83C9] transition-colors"
          >
            {category}
          </Link>
...
export default function ProductPageClient({
  ...
  const [similarProducts, setSimilarProducts] = useState<SimilarProduct[]>(initialSimilarProducts)
  const [reviews, setReviews] = useState<Review[]>([])
  const [reviewRating, setReviewRating] = useState<{ average: number; count: number }>({ average: 0, count: 0 })
  const [quantity, setQuantity] = useState(1)
  const [selectedVolume, setSelectedVolume] = useState<string>(() => {
    const firstVolume = initialProduct.volume_options
      ?.split(',')
  ...
  useEffect(() => {
    ...
    if (initialSimilarProducts.length === 0) {
      void fetch(
        `/api/products?category=${encodeURIComponent(initialProduct.category || '')}&limit=60&exclude=${encodeURIComponent(productId)}`
      )
        .then((response) => (response.ok ? response.json() : []))
        .then((data) => {
          setSimilarProducts(Array.isArray(data) ? data : data.products || [])
        })
        .catch(() => {
          // Recommendations are supplementary and must not block the product page.
        })
    }
  }, [initialProduct.category, initialSimilarProducts.length, productId])
